...
def sincnet_optims(model):
    return [optim.RMSprop(l.parameters(), 0.01, alpha=0.95) for l in model.layers()]
...
class SoftmaxConfig(LossConfig):
    ...
    def optimizer(self, model, task):
        if task == 'mnist':
            ...
            optimizers.append(optim.RMSprop(self.loss_module.parameters(), 0.01, alpha=0.95))
            schedulers = []
        elif task == 'sts':
...
class ContrastiveConfig(LossConfig):
    ...
    def optimizer(self, model, task):
        if task == 'mnist':
            ...
            optimizers = [optim.RMSprop(model.parameters(), lr=1e-6)]
            schedulers = []
        else:
            raise ValueError('Task must be one of mnist/speaker/sts')
        return base.Optimizer(optimizers, schedulers)

...
def contrastive_trainer():
    return ContrastiveTrainer(trainset, testset, device, margin=0.2, distance=CosineDistance())

def triplet_trainer():
    return TripletTrainer(trainset, testset, device, margin=0.2, distance=CosineDistance())
...
trainer.train(epochs=15, log_interval=60, train_accuracy=True)
